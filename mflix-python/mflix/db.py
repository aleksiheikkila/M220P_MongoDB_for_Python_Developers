...
def get_movies_by_country(countries):
    ...
    try:
        ...
        return list(db.movies.find({"countries": {"$in": countries}}, {"title": 1, "_id": 1})) #.limit(1))
...
def get_movies(filters, page, movies_per_page):
    ...
    argument will decide which page. (first page = page 0, it seems)
...
def get_movie(id):
    ...
    try:
        ...
        # sort movie comments from most recent to least recent:
        movie["comments"] = sorted(movie.get("comments", []), 
...
def add_comment(movie_id, user, comment, date):
    ...
    comment_doc = { "name": user.name,
                    "email": user.email,
                    "movie_id": ObjectId(movie_id),  # must put this as ObjectId
                    "text": comment,
                    "date": date }
...
def update_comment(comment_id, user_email, text, date):
    ...
    response = db.comments.update_one(
        { "_id": ObjectId(comment_id), "email": user_email},  # match these. Must be ObjectId...
...
def most_active_commenters():
    ...
    Construct a pipeline to find the users (emails) who comment the most on MFlix, sort
    ...
    # Read Concern "majority" to make sure the data we read has been majority-committed
